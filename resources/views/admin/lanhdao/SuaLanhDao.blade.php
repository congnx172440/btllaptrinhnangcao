...
@section('content')
    ...
    <div class="text-center">
        <h1 class="h4 text-gray-900 mb-4">SỬA THÔNG TIN LÃNH ĐẠO</h1>
    </div>
    <form class="user" action="admin/lanhdao/sua/{{$lanhdao->id}}" method="post">

        <input type="hidden" name="_token" value="{{csrf_token()}}">
        <hr />
        <div class="form-group">
            <h6 class="col-sm-4">Nhập chức vụ lãnh đạo mới</h6>
            <input type="text" class="form-control form-control-user"
                   placeholder="Nhập tên chức vụ" name="ten_chuc_vu" value="{{$lanhdao->ten_chuc_vu}}">
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control" style="font-size: 0.8rem;border-radius: 10rem;height: 3.1rem;"
                ...
                @foreach($giangvien as $tl)
                    <option
                        @if($lanhdao->id_giang_vien==$tl->id)
                        {{"selected"}}
                        @endif
